...
//		Document doc = Jsoup.parse(HtmlFetch.getHttpHTML(url));
		...
		/*String url_path = args[0];
		String output_path=args[1];*/
		String url_path = "G:/rss/url_list.txt";
		String output_path="G:/rss_result/";
		long star_time = System.currentTimeMillis();
		
		try{
			String cur_url;
			String cur_content;
			BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(new File(url_path))));
			while((cur_url=br.readLine())!=null){
				System.out.println(cur_url);
				Document doc = Jsoup.parse(HtmlFetch.getHttpHTML(cur_url, "UTF8"));
				BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(output_path+"/"+System.currentTimeMillis()+".txt"))));
				cur_content = performExtraction(doc);
				bw.write(cur_content);
				bw.close();
			}
			br.close();
		}catch(Exception e){
